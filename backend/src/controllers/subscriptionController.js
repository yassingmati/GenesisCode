...
exports.subscribe = async (req, res) => {
  try {
    ...
    if (!planId) {
      console.warn('Subscribe: planId manquant', { userId });
    ...
    if (!plan) {
      console.warn('Subscribe: Plan introuvable', { planId, userId });
    ...
    if (!plan.active) {
      console.warn('Subscribe: Plan inactif', { planId, userId });
      return res.status(400).json({ 
        success: false,
    ...
    if (!user) {
      console.warn('Subscribe: Utilisateur introuvable', { userId });
      return res.status(404).json({ 
        success: false,
