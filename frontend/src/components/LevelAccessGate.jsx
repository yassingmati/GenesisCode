...
const LevelAccessGate = ({ 
  ...
  const checkAccess = async () => {
    try {
      ...
      // Priorité: endpoint unifié si pathId + levelId disponibles
      if (pathId && levelId) {
        try {
          const url = API_CONFIG.getFullUrl(
            API_CONFIG.ENDPOINTS.ACCESS_CHECK({ pathId, levelId })
          );
          const res = await fetch(url, { headers: API_CONFIG.getDefaultHeaders() });
          if (res.ok) {
            const data = await res.json();
            const a = data.access || { hasAccess: false };
            setAccess(a);
            if (a.hasAccess) onAccessGranted && onAccessGranted(a);
            return;
          } else {
            const err = await res.json().catch(() => ({}));
            setAccess({
              hasAccess: false,
              reason: err.reason || err.code || err?.data?.reason || 'no_access'
            });
            return;
          }
        } catch (e) {
          // fallback sur logique catégorie
          console.warn('[LevelAccessGate] unified access check failed, fallback to category API');
        }
      }

      // Fallback: logique historique par catégorie
    ...
    <GateContainer>
      ...
      <Description>
        ...
        {access?.reason === 'plan_not_covering_path' &&
          'Votre abonnement actuel ne couvre pas ce parcours.'
        }
        {!access && 
