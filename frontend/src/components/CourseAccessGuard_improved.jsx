      ...
      try {
        ...
        if (response.ok) {
          ...
          setAccess({
            ...
            canView: false,
            canInteract: false,
            reason: errorData.access?.reason || 'no_access',
          ...
          <div className="blocked-actions">
            ...
            {access?.availablePlans && access.availablePlans.length > 0 && (
              <div className="available-plans">
                <p className="plans-label">{t('availablePlans') || 'Plans disponibles'} :</p>
                <div className="plans-list">
                  {access.availablePlans.slice(0, 3).map((plan) => (
                    <div key={plan._id} className="plan-item">
                      <span className="plan-name">{plan.name}</span>
                      <span className="plan-price">
                        {plan.priceMonthly ? `${(plan.priceMonthly / 100).toFixed(2)} TND` : t('free') || 'Gratuit'}
                      </span>
                    </div>
                  ))}
                </div>
